...
QStringList TestUpcA::encodedBlock(QList< barcodeEngine::Symbol > symbolList, QString encoding)
{
  QStringList temp;
  ...
  QList<barcodeEngine::Symbol>::const_iterator _first = symbolList.begin();
  QList<barcodeEngine::Symbol>::const_iterator _last = symbolList.end();
...
void TestUpcA::initTestCase()
{
 ...
 void TestUpcA::defaultConstructor()
 {
   ...
   QList<barcodeEngine::Symbol> symbols = barcodeEngine::Symbol().parse(finalString);  
   ...
   switch(symbolLength) {
     ...
     case 14:
	  ...
	  QList<barcodeEngine::Symbol> extendedSymbols(symbols.mid(12));  
